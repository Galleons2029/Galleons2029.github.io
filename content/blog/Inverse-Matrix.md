...
## The condition a matrix has an inverse

This is the first question we ask about a square matrix:
Is A invertible? We don't mean that we immediately calculate $A^{-1}$. In most problems we never compute it! Here are six "notes" about $A^{-1}$.
...
4. (Important) Suppose there is a nonzero vector x such that Ax = 0. Then A cannot have an inverse. No matrix can bring O back to x. **If A is invertible, then Ax = 0 can only have the zero solution $x = A^{-1} 0 = 0$.**
...
Row exchanges are certainly allowed in Gauss-Jordan and don't change the sign.
